...
function App() {
  ...
  const [link3, setlink3] = useState(
    `https://api.weatherapi.com/v1/search.json?key=0cb021f26c2e44d4be562400220110&q=Lodon`
  );
  useEffect(() => {
    axios(link).then((e) => {
      setdata(e.data);
  ...
  useEffect(() => {
    if (name == "") {
      setlink3(
        `https://api.weatherapi.com/v1/search.json?key=0cb021f26c2e44d4be562400220110&q=London`
      );
    }
    axios(link3).then((e) => {
      lets(e.data);
